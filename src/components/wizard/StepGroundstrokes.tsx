...
import { GROUNDSTROKES_SERIES } from '../../lib/protocol';
...
export const StepGroundstrokes: React.FC<Props> = ({ state, setSeriesResult, onNext, onPrev }) => {
  const [activeIdx, setActiveIdx] = useState(0);
  const activePlayer = state.players[activeIdx];

  const isDoneForPlayer = (pidx: number) =>
    GROUNDSTROKES_SERIES.every(s =>
      state.players[pidx].series.some(r => r.testType === 'groundstrokes' && r.seriesIndex === s.seriesIndex)
    );

  const completedFlags = state.players.map((_, i) => isDoneForPlayer(i));
  const canContinue = completedFlags.every(Boolean);

  const activeDone = GROUNDSTROKES_SERIES.filter(s =>
    activePlayer.series.some(r => r.testType === 'groundstrokes' && r.seriesIndex === s.seriesIndex)
  ).length;
  ...
  return (
    <div className="space-y-4">
      ...
      <Card
        title={`Groundstrokes${playerLabel}`}
        subtitle="20 serie × 10 palle — alternando FH incrociato / BH incrociato"
      >
        <SeriesInputBlock
          specs={GROUNDSTROKES_SERIES}
      ...
      {!canContinue && (
        <p className="text-center text-xs text-gray-400">
          {state.playerCount > 1
            ...
            : `Inserisci tutte le ${GROUNDSTROKES_SERIES.length} serie per continuare (${activeDone}/${GROUNDSTROKES_SERIES.length})`
          }
        </p>
      )}
    </div>
